...
class AFHMM(Disaggregator):
    ...
    def disaggregate_thread(self, test_mains, index, d, pi_s_vec: np.array, means_vec: np.array, transmat_vec: np.array):

        # A threads that does disaggregation
        pi_s_vector = np.array(pi_s_vec, dtype=np.float64)
        means_vector = np.array(means_vec, dtype=np.float64)
        transmat_vector = np.array(transmat_vec, dtype=np.float64)
        s_ = np.array([], dtype=np.float64)
        ...
        for epoch in range(6):
            ...
            if epoch % 2 == 1:
                ...
                if flag == 0:
                    ...
                    for appliance_id in range(self.num_appliances):
                        state_vector = cvx.Variable(shape=(len(test_mains), self.default_num_states),
                        ...
                        for t in range(len(test_mains)):
                            matrix = cvx.Variable(shape=(self.default_num_states, self.default_num_states),
                        ...
                        for t in range(0, len(test_mains)):  # 6e
                            for i in range(self.default_num_states):
                                constraints += [cvx.sum(((cvx_variable_matrices[appliance_id][t]).T)[i]) ==
                                                cvx_state_vectors[appliance_id][t][i]]
                        # Constraint 6d
                        for t in range(1, len(test_mains)):  # 6d
                            for i in range(self.default_num_states):
                                constraints += [cvx.sum(cvx_variable_matrices[appliance_id][t][i]) ==
                ...
                prob.solve(solver=cvx.ECOS_BB, verbose=False, warm_start=True)
    ...
    def disaggregate_chunk(self, test_mains_list):
        ...
        for test_mains in test_mains_list:
            ...
            for test_block in range(int(math.ceil(len(test_mains_big) / self.time_period))):
                ...
                t = Process(target=self.disaggregate_thread, args=(
                    test_mains, test_block, d,
                    np.array(self.pi_s_vector), np.array(self.means_vector), np.array(self.transmat_vector)
                ))
                threads.append(t)
