...
def create_model(input_window_length, ncols):
    ...
    input_layer = tf.keras.layers.Input(shape=(input_window_length, ncols))
    reshape_layer = tf.keras.layers.Reshape((1, input_window_length, ncols))(input_layer)
    conv_layer_1 = tf.keras.layers.Convolution2D(filters=30, kernel_size=(10, ncols), strides=(1, 1), padding="same", activation="relu")(reshape_layer)
